<template>
  <div>
    ...
    <el-dialog title="分配权限"
               :visible.sync="setRightDialogVisible" width="30%" @close="setRightClose">
      <!--树型控件-->
...
export default {
  ...
  methods: {
    ...
    async removeRightById(role, rightId) {
      // 弹框提示用户是否删除
      const confrimResult = await this.$confirm('此操作将永久删除该用户权限, 是否继续?', '提示', {
    ...
    getLeafKeys(node, arr) {
      // 如果当前节点不包含children属性,则当前节点不是三级权限
      if (!node.children) {
